...
class Controller
{
     ...
     *
     * @param string $controllerPrefix The controllers prefix of the route to redirect to.
    ...
    protected function authorizationFilterAllows($role=[])
    {
        $auth = new Auth();
        if(!$auth->IsLoggedInAs($role))
        {
            header('Location: '.constant('INVALID_ACCESS_ROUTE'));
        }
    }
